...
(function () {
  ...
  var __ = wp.i18n.__;
  var metabox = document.querySelector('#social-planner-metabox > .inside');
  var $ = jQuery; // Stop if settings element not exists.
  ...
  var saveMetaBox = function saveMetaBox() {
    var postID = document.getElementById('post_ID');
    var data = $($("#social-planner-metabox").parents("form")[0]).serialize() + '&action=' + config.action + '&nonce=' + config.nonce + '&post=' + postID.value + '&handler=save';
    $.ajax({
      url: ajaxurl,
      type: 'POST',
      data: data
