...
class Scheduler
{
	...
	public static function schedule_tasks($tasks, $post)
	{
		foreach ($tasks as $key => &$task) {
			...
			if (empty($task['targets'])) {
				...
				throw new Exception(__("No targets selected", "social-planner"));
			}

			$task_is_ok = apply_filters('social_planner_task_validation', true, $task, $key, $post->ID);
			if ($task_is_ok != true) {
				self::unschedule_task($key, $post->ID);
	...
	{
		if ('do-not-send' === $task['date']) {
			throw new Exception(__("No datetime asigment in task", "social-planner"));
		}
		...
		if (isset($task['datetime'])) {
			$datetime = strtotime($task['datetime']);
			$month = date('m', $datetime);
			$day = date('d', $datetime);
			$year = date('Y', $datetime);
			if (wp_checkdate($month, $day, $year, $datetime)) {
